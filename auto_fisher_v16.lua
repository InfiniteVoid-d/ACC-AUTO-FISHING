...
-- =============================================
-- CONFIG (Tuned for Blatant Peak Performance)
-- =============================================
local Config = {
    Mode = "Blatant",
    ...
    BlatantHybridDelay = 3.5,    -- Delay before direct catch event fires
    
    -- Instant settings (Adjustable via UI input box):
    InstantCatchDelay = 2.0,     -- Delay before upvalue catch completes
    
    BlatantRecastDelay = 0.05,   -- Lowered to 50ms for near-instant recasting
    ...
    -- Shared Safety Recast (Lowered to 8s to prevent soft-locks on failed casts)
    SafetyRecastTime = 8.0,
    EscapeRecastDelay = 0.1,     -- Shrunk to 100ms
}
...
delayInputBox.PlaceholderText = "2.0"
...
local function doCast()
    ...
    if Config.Mode == "Blatant" and Config.BlatantParallelCasts then
        ...
        task.wait(0.02) -- Optimized split delay from 50ms to 20ms for faster casts
        task.spawn(function()
            pcall(Fish.FireServer, Fish, "CastRod", castVal)
        end)
    else
