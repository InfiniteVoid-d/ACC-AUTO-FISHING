...
local function getPackPrice(packName)
    local price = nil
    pcall(function()
    ...
    autoBuyPacksThread = task.spawn(function()
        ...
        local conn = packFolder.ChildAdded:Connect(function(child)
            ...
            if plotNum == myPlot.Name then
                ...
                local packPrice = getPackPrice(packType) or getPackPriceFromModel(child)
                
                if packPrice and playerYen < packPrice then
                    setDebug(string.format("Skip %s: Need %d Yen (Have %d)", packType, packPrice, playerYen))
                    return
                end
