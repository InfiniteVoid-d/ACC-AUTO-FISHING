-- =============================================
-- Auto Fisher v32 - Smooth Auto Collect Money
...
local function startAutoCollectLoop()
    ...
    collectThread = task.spawn(function()
        while Config.AutoCollect do
            ...
            -- 2. Card Binder Sweep (Runs dynamically every 20 seconds)
            if tick() - lastCardCollect > 20 then
                lastCardCollect = tick()
                pcall(function()
                    local CardRemote = ReplicatedStorage.Remotes:FindFirstChild("Card")
                    if CardRemote then
                        -- Flip left 8 times to reset to Page 1
                        for i = 1, 8 do
                            if not Config.AutoCollect then break end
                            CardRemote:FireServer("Page", "LeftArrow")
                            task.wait(0.04) -- Light delay to prevent key-spam lag
                        end
                        
                        -- Sweep Pages 1 to 8
                        for page = 1, 8 do
                            if not Config.AutoCollect then break end
                            setDebug("Sweeping Page " .. page .. "/8...")
                                    ...
                                    if left then
                                        for _, slot in ipairs(left:GetChildren()) do
                                            ...
                                            if slot:IsA("BasePart") and tonumber(slot.Name) then
                                                ...
                                                task.wait(0.04) -- Lag protection delay!
                                            end
                                        end
                                    end
