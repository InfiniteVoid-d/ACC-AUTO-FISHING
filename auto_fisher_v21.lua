-- =============================================
-- Auto Fisher v21 - Smart Blatant Pipelining
...
local Config = {
    ...
    InstantCatchDelay = 1.5,     -- Default delay before catch (e.g. 1.5s struggle)
    SlowReelThreshold = 0.91,     -- Target struggle percentage (default 91% or 0.91)
    
    BlatantRecastDelay = 0.3,    -- Cast spam interval (default 0.3s between casts)
...
delayInputBox.PlaceholderText = "1.5"
...
local function updateModeUI()
    ...
    if isBlatant then
        ...
        elseif s == "blatant" then
            ...
            delayInputBox.Text = tostring(Config.InstantCatchDelay)
            thresholdLabel.Text = "Cast Spam Interval (s):"
...
thresholdInputBox.FocusLost:Connect(function(enterPressed)
    ...
    if val then
        ...
        elseif Config.BlatantStrategy == "blatant" then
            val = math.max(val, 0.01) -- Minimum 10ms recast
            Config.BlatantRecastDelay = val
            setDebug("Spam interval set to " .. val .. "s")
        end
...
-- =============================================
-- PURE BLATANT PIPELINED CAST/CATCH SPAM LOOP
-- =============================================
local function startBlatantLoop()
    ...
    blatantThread = task.spawn(function()
        while autoFishing and Config.BlatantStrategy == "blatant" do
            ...
            setDebug("Pipelining CastRod...")
            
            -- Fire the cast
            local castVal = Config.BlatantCastValue
            pcall(Fish.FireServer, Fish, "CastRod", castVal)
            
            -- Recast timing is handled by this loop spamming CastRod
...
local function startAutoFish()
    ...
    connection = Fish.OnClientEvent:Connect(function(eventType, fishName, amount)
        ...
        if eventType == "StartFishing" then
            if Config.BlatantStrategy == "blatant" then
                -- Event-driven blatant catcher: wait for the server-side bite event
                task.spawn(function()
                    local delay = getCatchDelayForFish(fishName)
                    setStatus("🔥 BLT: Reeling " .. tostring(fishName), Color3.fromRGB(0, 220, 150))
                    setDebug("Catching in " .. delay .. "s...")
                    task.wait(delay)
                    if not autoFishing or Config.BlatantStrategy ~= "blatant" then return end
                    pcall(Fish.FireServer, Fish, "FishCaught")
                    fishCaught = fishCaught + 1
                    updateStats()
                end)
        ...
        elseif eventType == "FishEscaped" then
            if Config.BlatantStrategy ~= "blatant" then
                ...
                setStatus("Escaped! (Too fast)", Color3.fromRGB(255, 100, 0))
            end
